...
# 0.1 Syntax check — use py_compile which handles file encoding correctly
import py_compile, tempfile, shutil
try:
    py_compile.compile(graph_path, doraise=True)
...
# Parse for AST analysis — use source bytes to handle non-ASCII
...
REQUIRED_TOOLS = ['get_expenses','get_pantry','update_pantry','log_expense',
                  'get_preferences','save_preference','send_weekly_email','save_meal']
...
SAT_CHICKEN_GRAVIES = ["chole", "aloo gobi gravy", "chana masala"]  # non-dal only for Saturday
...
# SYSTEM string integrity — catches the unterminated string bug
if 'TOOLS = [' not in code: fail("TOOLS list missing — SYSTEM string may be unterminated!")
else: ok("TOOLS list present after SYSTEM string")
if code.count('"""') % 2 != 0: fail("Odd number of triple-quotes — unterminated string!")
else: ok("Triple-quote count balanced")
...
is_monsoon = dt_module.datetime.now().month in MONSOON_MONTHS
DAY_TYPE = {
    0: "chicken",
...
def file_contains(filename, text):
    # Look relative to test file location (backend/)
...
checks = {
    ...
    "Monsoon DAY_TYPE":         "is_monsoon" in code,
    "save_meal tool":           "save_meal" in code,
    "pepper chicken":           "pepper chicken" in code,
    "sea bass":                 "sea bass" in code,
    "Monsoon mode":             "MONSOON_MONTHS" in code,
    "save_meal tool":           "save_meal" in code,
    "Friday cron in railway":   file_contains('railway.json','0 12 * * 5'),
    "weekly-plan in main":      file_contains('main.py','weekly-plan'),
    "preferences router":       file_contains('main.py','preferences_router'),
}
